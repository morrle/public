...
samplesInstall() {
  ...
  if [ -d first-network ]; then
    ...
    echo "===> Checking out release-1.1 branch of hyperledger/fabric-samples"
    ...
    echo "===> Cloning hyperledger/fabric-samples repo and checkout release-1.1"
    git clone -b master https://github.com/hyperledger/fabric-samples.git && cd fabric-samples && git checkout release-1.1
  fi
}
...
binaryIncrementalDownload() {
      ...
      if [ -z "$rc" ] || [ $rc -eq 33 ] || [ $rc -eq 2 ]; then
          ...
          echo "==> File downloaded.Skip verifying the md5sum..."
           localMd5sum="md5"
           remoteMd5sum="md5"
...
binariesInstall() {
  ...
  binaryDownload ${BINARY_FILE} http://github-mirror.bugkiller.org/hyperledger/fabric/releases/download/v${VERSION}/${BINARY_FILE}
  if [ $? -eq 22 ]; then
     echo
  ...
  binaryDownload ${CA_BINARY_FILE} http://github-mirror.bugkiller.org/hyperledger/fabric-ca/releases/download/v${CA_VERSION}/${CA_BINARY_FILE}
  if [ $? -eq 22 ]; then
     echo
